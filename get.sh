...
rsc_fwv4()
{
	...
	echo "/ip firewall address-list remove [/ip firewall address-list find list=IRAN-IP]";
...
rsc_respinav4()
{
	echo ":do { add address=5.160.0.0/16 list=IRAN-IP} on-error={}";
	echo ":do { add address=46.209.0.0/16 list=IRAN-IP} on-error={}";
	echo ":do { add address=77.104.64.0/18 list=IRAN-IP} on-error={}";
}

rsc_intranetv4()
{
	echo ":do { add address=10.0.0.0/8 list=IRAN-IP} on-error={}";
}
...
if [ "$1" != "v6" ]
then
	rsc_address_add "$( echo $output | jq -r $filterv4 )" IRAN-IP v4
fi
